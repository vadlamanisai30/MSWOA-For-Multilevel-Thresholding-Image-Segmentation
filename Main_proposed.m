...
close all;
   ...
   img = imread('building.jpg');  % Replace with your image file
levels = 8;  % Number of thresholds
...
method='kapur';
    ...
    % Optimization loop
    for t = 1:max_iter
        a = nonlinear_convergence_factor_proposed(t, max_iter, u);
...
fprintf('FSIM: %.4f\n', fsim_value);
if size(img,3) == 1
[n_countR, x_valueR] = imhist(img(:,:,1));
end
Nt = size(img,1) * size(img,2); 
for i = 1:256
    if size(img,3) == 1  
